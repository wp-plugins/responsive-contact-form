...
<div class="wrap"> <img src="<?php echo plugins_url();?>/responsive-contact-form/images/augustinfotech.jpg" class="icon32" />
  <h2>
    <?php _e('Responsive Contact Form Settings','aicontactform');?>
  </h2>
  <div class="postbox" id="poststuff">
    <h3 class="title">
      <?php _e('Settings','aicontactform');?>
    </h3>
    <form method="post" action="options.php" name="AIGolbalSiteOptions">
      ...
      <table class="form-table" id="form-settings">
        <tbody>
          ...
          <tr>
            ...
            <td class="field-status"><input type="checkbox" name="ai_enable_captcha" id="ai_enable_captcha" <?php if(esc_attr(get_option('ai_enable_captcha'))=="on"){echo "checked";} ?>  /></td>
      ...
      <table class="form-table" id="email-settings">
        <tbody>
          ...
          <tr>
            <th><label for="ai_reply_user_message">
                <?php _e('Reply Message for User:','aicontactform');?>
              </label></th>
            <td><?php /* <input type="text" name="ai_reply_user_message" class="regular-text" value="<?php echo esc_attr(get_option('ai_reply_user_message'));?>"> */?>
              <textarea name="ai_reply_user_message" rows="5" cols="49" class="regular-text"><?php echo esc_attr(get_option('ai_reply_user_message'));?></textarea></td>
            <td><?php _e('<strong>Note:</strong> Default Reply Message " Thank you for contacting us...We will get back to you soon... " will be used.','aicontactform');?></td>
          </tr>
          <tr>
            <th><label for="ai_custom_css">
          ...
          <tr>
            <td colspan="3" align="center"><?php _e('<strong>Note:</strong> You can add <strong> [ai_contact_form] </strong> shortcode where you want to display contact form in pages.','aicontactform');?>
              <?php  _e(' <br/> OR  You can add <strong> &lt;&#63;php do_shortcode("[ai_contact_form]"); &#63;&gt;</strong> shortcode in any template.','aicontactform');?>
              <?php  _e(' <br/> OR  You can add <strong> &lt;&#63;php echo do_shortcode("[ai_contact_form]"); &#63;&gt;</strong> shortcode in any template.','aicontactform');?></td>
          </tr>
          <tr>
            <td colspan="3"></td>
          </tr>
